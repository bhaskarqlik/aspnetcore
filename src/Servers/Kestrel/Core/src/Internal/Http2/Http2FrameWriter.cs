        ...
        while (await _channel.Reader.WaitToReadAsync())
        {
            ...
            while (_channel.Reader.TryRead(out var producer) && !producer.CompletedResponse)
            {
                try
                {
                    ...
                    if (actual < 0)
                    {
                        // Console.WriteLine($"{DateTime.UtcNow.ToString("o")}, AspNetKestrel: {this._connectionId} StreamId={stream.StreamId} WriteToOutputPipe actual is {actual} after CheckStreamWindow({buffer.Length}) resetting to 0");
                        actual = 0;
                    }
                    ...
                    if (actual < 0)
                    {
                        // Console.WriteLine($"{DateTime.UtcNow.ToString("o")}, AspNetKestrel: {this._connectionId} StreamId={stream.StreamId} WriteToOutputPipe actual is {actual} after CheckConnectionWindow({buffer.Length}) resetting to 0");
                        actual = 0;
                    }
